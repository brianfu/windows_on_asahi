...
- To start setup:
  ```
    Manual Registry Bypass (During Install):
      If you've already started a clean install from a standard USB and hit the "This PC can't run Windows 11" screen:
    Open CMD: Press Shift + F10 to open a command prompt.
    Launch Regedit: Type regedit and hit Enter.
    Create Keys: Navigate to HKEY_LOCAL_MACHINE\SYSTEM\Setup.
    Add LabConfig: Right-click Setup > New > Key. Name it LabConfig.
    Add DWORDs: Inside LabConfig, create three DWORD (32-bit) values and set their data to 1:
      BypassTPMCheck
      BypassSecureBootCheck
      BypassRAMCheck
    Continue: Close the registry editor and CMD. Go back one step in the installer, then forward; the error should disappear.
  ```
...
# Post Installation
Shut down the VM as usual to not make Windows scared (don’t just exit the win11.sh script). To boot it again after virtio tools installation, run `win11_run.sh`.

Ensure the virtio-win-guest-tools.exe package is installed in the guest.
